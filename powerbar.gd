...
const segDistance: float = 60 #maximum distance between the segments
...
func stretch(value):
	...
	var dist = percent * segDistance * -1
	segment4.position.y = dist
	segment3.position.y = dist * 2
	segment2.position.y = dist * 3
	segment1.position.y = dist * 4
	triangle.position.y = dist * 5
