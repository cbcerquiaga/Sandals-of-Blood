...
func _protect_goalie(delta):
	if not goalie or opponents.is_empty(): return
	
	# Stay near goalie while looking for threats
	var protect_pos = goalie.global_position + Vector2(100, 0)
...
func _get_open(delta):
	if teammates.is_empty(): return
	
	# Find position away from opponents and open to passes
	var avg_teammate_pos = Vector2.ZERO
	for teammate in teammates:
		avg_teammate_pos += teammate.global_position
	avg_teammate_pos /= teammates.size()
	
	var open_pos = global_position
	var best_score = -INF
	
	# Check 8 possible directions
	for angle in range(0, 360, 45):
		var test_pos = global_position + Vector2.RIGHT.rotated(deg_to_rad(angle)) * 200.0
		var score = _calculate_openness_score(test_pos, avg_teammate_pos)
		
		if score > best_score:
			best_score = score
			open_pos = test_pos
	
	velocity = (open_pos - global_position).normalized() * move_speed
	move_and_slide()
...
func _calculate_openness_score(pos: Vector2, teammate_center: Vector2) -> float:
	var score = 0.0
	
	# Distance from opponents
	for opponent in opponents:
		score += pos.distance_to(opponent.global_position) * 0.5
	
	# Alignment with teammates
	score -= pos.distance_to(teammate_center) * 0.3
	
	# Field position bonus
	if pos.x > get_viewport_rect().size.x * 0.5:
		score += 50.0  # Prefer attacking positions
	
	return score

func set_state(new_state: ForwardState):
...
func _on_teammate_goalie_spotted(goalie_node: Goalie):
	goalie = goalie_node

func _on_opponent_goalie_spotted(goalie_node: Goalie):
